...
3. **Stop** → `stop-gate.sh`, the flagship:
   ...
   - Tier 2 (opt-in via `FABLE_STOP_JUDGE=1`): pipe the last message to `claude -p --model claude-haiku-4-5` (--bare removed 2026-07-02: requires API-key auth; inherits session auth instead) with a 10-line rubric → verdict JSON → block/allow. ~2–5s latency per stop; off by default.
