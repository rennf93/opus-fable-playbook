...
- **`run-probe.sh`** — runs one probe headless: `claude -p "$(cat probe)" --model <m> --output-format json` with either `--bare` (vanilla baseline) or `--plugin-dir <repo>` + profile settings (playbook candidate); saves transcript JSON.
